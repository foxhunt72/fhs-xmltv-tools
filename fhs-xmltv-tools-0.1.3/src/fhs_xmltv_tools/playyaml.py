...
def play_command_loadxml(task):
    ...
    with config.CONSOLE.status(
        f"Loading...{file} to store {store}", spinner="dots"
    ):
...
def play_command_only_channels(task):
    ...
    with config.CONSOLE.status(
        f"Removing channels from store {store}", spinner="dots"
    ):
...
def play_command_add(task):
    ...
    with config.CONSOLE.status("Join channels...", spinner="dots"):
        config.STORE[store] = join_channels(config.STORE[store], config.STORE[add_store])

    with config.CONSOLE.status("Join programs...", spinner="dots"):
        config.STORE[store] = join_programs(config.STORE[store], config.STORE[add_store])
...
def play_command_change_timezone(task):
    ...
    with config.CONSOLE.status("Search and replace in start/stop program time...", spinner="dots"):
        config.STORE[store] = search_and_replace_in_start_and_stop(config.STORE[store], search, replace)

    print(f"Search and replace in start/stop time programs: store {store} search:{search} / replace:{replace}")
    return True
